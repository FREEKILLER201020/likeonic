...
function Start($message, $bot) {
	...
	$bot->sendMessage($message->getChat()->getId(), $answer, false, null, null, $keyboard);
}; // function LastUserMessage($chat_id, $user_id, $back) {; // 	$query = "select id,message from messages_history" . $db_name . " where user_id=$user_id and chat_id=$chat_id order by id,timemark desc limit $back";; // 	$result = pg_query($query);; // 	while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {; // 		$res = $line["message"];; // 	}; // 	return $res;; // }; // function SetState($message, $bot, $state) {; // 	// $nick = $message->getFrom()->getUsername();; // 	// $name = $message->getFrom()->getFirstName();; // 	$query = "UPDATE users" . $db_name . " set chat_state='$state' where id={$message->getFrom()->getId()} and chat_id={$message->getChat()->getId()};\n";; // 	$result = pg_query($query) or $answer = 'Не удалось соединиться: ' . pg_last_error();; // }; // function GetState($message, $bot) {; // 	$query = "SELECT distinct on (id) id,chat_state from users" . $db_name . " where id={$message->getFrom()->getId()} and chat_id={$message->getChat()->getId()} order by id desc";; // 	$result = pg_query($query);; // 	while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {; // 		$state = $line["chat_state"];; // 	}; // 	return $state;; // };; // class FightClassNot {; // 	// public $id;; // 	public $attacker; // id атакующего клана; // 	public $attacker_id; // id атакующего клана; // 	public $defender; // id защищающегося клана; // 	public $defender_id; // id защищающегося клана; // 	public $from;; // 	public $to;; // 	public $declared; // время, когда был объявлен бой; // 	public $resolved; // вермя, когда состаится бой; // 	public $ended; // вермя, когда состаится бой; // 	public $winer_id;; // 	// public $in_progress; // флаг, активен ли бой; // 	public $winer;; // 	public $was = 0;; // 	public function __construct($a, $d, $f, $t, $de, $r, $w, $end, $id1, $id2, $id3) {; // 		$this->attacker = $a;; // 		$this->defender = $d;; // 		$this->from = $f;; // 		$this->to = $t;; // 		$this->declared = $de;; // 		$this->resolved = $r;; // 		$this->winer = $w;; // 		$this->ended = $end;; // 		$this->attacker_id = $id1;; // 		$this->defender_id = $id2;; // 		$this->winer_id = $id3;; // 	}; // }
?>